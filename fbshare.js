/*1298698703,177283434,JIT Construction: v347607,en_US*/

if (!window.FB) {FB = {};}

if(!FB.dynData) { 
	FB.dynData = {	"site_vars":
						{	"canvas_client_compute_content_size_method":1,
							"use_postMessage":0,"use_xdProxy":0,
							"use_ui_server":1,
					...
					"resources":
						{	"base_url_format":"http:\/\/{0}.facebook.com\/",
							...
							"api_channel":1298439705,
							"api_server":1298439705,
							"www_channel":1298439632,
							"xd_proxy":1298439527,
...
if(!window.FB.isSecure)
	window.FB.isSecure=function(){
		return (
			...
			(window.name.indexOf('_fb_https')>-1);};
...
if(!window.FB.Share){
	FB.Share={
		...
		displayBox:function(a,d){
			if(typeof(a.fb_count)=='number'&&a.fb_count>=d)
				for(var c=1;c<=2;c++){
					var b=a.firstChild.childNodes[c];
					b.className=b.className.replace('fb_share_no_count','');
					if(c==2)
						b.lastChild.innerHTML=this.pretty(a.fb_count);
				}
		},
		renderButton:function(c){
			...
			c.onclick=function(){
				if(!c.fb_clicked){
					c.fb_count+=1;FB.Share.displayBox(this,1);
					c.fb_clicked=true;}
			...
			if(h.indexOf('count')>=0){
				...
				var b='<span class=\'fb_share_count_nub_'+f+' fb_share_no_count\'></span>';b+='<span class=\'fb_share_count fb_share_no_count'+' fb_share_count_'+f+'\'>'+'<span class=\'fb_share_count_inner\'>&nbsp;</span></span>';
		...
		fetchData:function(){
			...
			var a=[];
			for(var b=0;b<this.urlsA.length;++b)
				a.push('"'+this.urlsA[b].replace('\\','\\\\').replace('"','\\"')+'"');
			c.src=this.addQS(
				(FB.isSecure()?'https:':'http:')+'//api.facebook.com/restserver.php',{
																						v:'1.0',
		...
		_onFirst:function(){
			...
			window.fb_sharepro_render=function(c){
											for(var d=0;c&&d<c.length;d++)
												FB.Share.results[c[d].url]=c[d];
											FB.Share.renderAll();};
			this.renderPass();
			this.scanner=setInterval(FB.Share.renderPass,700);
			if(window.attachEvent){window.attachEvent("onload",FB.Share.stopScan);}
